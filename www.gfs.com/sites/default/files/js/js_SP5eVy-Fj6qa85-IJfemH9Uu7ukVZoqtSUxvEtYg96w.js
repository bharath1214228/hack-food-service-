...
(function($){
  ...
  var _augmentGoogleMaps = function () {
   ...
   * @param {Object} dto The data transfer object from the JSON data file that represents the location.
   ...
   */
  function Location(serviceAreas, dto, icons, map){
    // Private self for closures.
    var _self = this;
    
    // Public members.
    /**
     * {Node} The data transfer object from the JSON data file that represents the location.
     */
    this.dto = dto;
    /**
     * {String} The name of the location. 
     */
    this.name = this.dto.title;
    /**
     * {String} The address of the location. 
     */
    this.address = this.dto.address_rendered;
    /**
     * {google.maps.LatLng} The latitude and longitude of the location. 
     */
    this.latlng = new google.maps.LatLng(this.dto.latitude, this.dto.longitude);
    /**
     * {String} The type of the location, such as "dc". 
     */
    this.type = this.dto.location_type;
    /**
     * {String} The details of the location (HTML).
     */
    ...
    (function(){
      ...
      infoWindowText += '<div><a href="' + drupalSettings.bac_url + '">' + Drupal.t('Become a customer') + '</a></div>';
   ...
   */
  function PolygonGroup(serviceAreas, xml, map){
    // Private self for closures.
    var _self = this;
    ...
    (function(){
      ...
      var polygonFilter = _getUrlParameter('polygonFilter');
      if (polygonFilter) {
   ...
   */
  function Message(serviceAreas, xml){
    // Private self for closures.
    var _self = this;
   ...
   */
  function Distribution(serviceAreas, xml, map){
    // Private self for closures.
    var _self = this;
  ...
  ServiceAreas = function(){
     ...
     */
    this.config = {
      maxInfoWindowWidth: 200,
      initialZoom: 3,
      maxZoom: 11,
      centerLatLng: new google.maps.LatLng(45.583289756006316, -90),
      locationUrl: '/' + drupalSettings.language + '/service/locations/area?_format=json',
      dataUrl: drupalSettings.gfs_locator_path + '/data/' + drupalSettings.site + '/service-areas-data_' + drupalSettings.language + '.xml',
      bufferDistance: 50
    };
    /**
    ...
    var _processData = function(data){
      ...
      $('service-areas > polygonGroups > polygonGroup', data).each(function(){
        var polygonGroup = new PolygonGroup(_self, this, _map);
      });
      
      // Create messages.
      $('service-areas > messages > message', data).each(function(){
        var message = new Message(_self, this);
      });
      
      // Create distributions.
      $('service-areas > distributions > distribution', data).each(function(){
        var distribution = new Distribution(_self, this, _map);
    ...
    /**
     * Processes the JSON locations into {@link Location} objects.
     * @param {Array} data An array of loaded location data transfer objects.
     * @private
     * @memberOf ServiceAreas
    ...
    var _processLocations = function(data){
      ...
      // Create locations.
      $.each(data, function(){
        var location = new Location(_self, this, _icons, _map);
        location.hide();
    ...
    var _processComplete = function(){
      ...
      if (_dtoLocations == null || _xmlData == null)
        return;
      ...
      if(drupalSettings.site == 'us') {
        $('#serviceAreasAreaType-restaurants').click();
        // $('#serviceAreasAreaType-chain-operators').click();
      }
      if(drupalSettings.site == 'ca') {
        $('#serviceAreasAreaType-restaurants').click();
      }
     ...
     * XML data file and JSON locations file.
     * @memberOf ServiceAreas
     * @name initialize
     */
    this.initialize = function(){
      ...
      var centerLatLng = _getUrlParameter('centerLatLng');
      var initialZoom = _getUrlParameter('initialZoom');
      if (centerLatLng)
    ...
    this.checkLocation = function(results, status){
      ...
      if (_self.currentDistribution == null) {
        console.log('Distribution area JSON data was null!');
      }
      ...
      if (latlng != null && _self.currentDistribution != null) {
    ...
    (function(){
      ...
      _icons.shadow = new google.maps.MarkerImage(drupalSettings.gfs_locator_path + '/images/marker-flag-shadow.png',
        new google.maps.Size(27, 25),
        sharedIconOptions.origin,
        sharedIconOptions.anchor);
      
      
      _icons.store = new google.maps.MarkerImage(drupalSettings.gfs_locator_path + '/images/marker-marketplace.png', sharedIconOptions.size, sharedIconOptions.origin, sharedIconOptions.anchor);
      _icons.dc = new google.maps.MarkerImage(drupalSettings.gfs_locator_path + '/images/marker-distribution.png', sharedIconOptions.size, sharedIconOptions.origin, sharedIconOptions.anchor);
      _icons.office = new google.maps.MarkerImage(drupalSettings.gfs_locator_path + '/images/marker-offices.png', sharedIconOptions.size, sharedIconOptions.origin, sharedIconOptions.anchor);
      _icons.address = new google.maps.MarkerImage(drupalSettings.gfs_locator_path + '/images/marker-epicenter.png');
  ...
  (function ($) {
    ...
    $.getScript('//maps.google.com/maps/api/js?key=' + drupalSettings.gfs_locator.google_maps_api_key + '&sensor=true&callback=_googleMapsLoaded&language=' + drupalSettings.short_language);
  }(jQuery));
})(jQuery);;
