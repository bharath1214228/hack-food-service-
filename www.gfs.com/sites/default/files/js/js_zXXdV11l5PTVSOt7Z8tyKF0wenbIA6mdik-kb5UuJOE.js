...
(function ($) {
  ...
  // Hide the selector exposed view filter on blur.
  $(document).click(function(event) { 
    if(!$(event.target).is('.market-update-selector__items-wrapper') && (!$(event.target).is('.btn--market-update-selector') && !$(event.target).is('.btn--market-update-selector i')) && !$(event.target).is('.market-update-selector__items-wrapper .form-item')) {
      $('.market-update-selector__items-wrapper').hide();
    }
  });

  Drupal.behaviors.GFSMarketUpdateSelector = {
    attach: function(context, settings) {

      // Toggle the selector exposed view filter.
      $('.btn--market-update-selector').once('market-update-selector').each(function() {
      ...
      var $category_container = $('.market-update-selector__items .fieldset-wrapper > .form-checkboxes');

      setTimeout(function(){
        $category_container.each(function() {

        // Show which sections are being displayed above the content.
        var labelsChecked = [];

        $('.selected-market-updates').remove();
        $category_container.find('input[type="checkbox"]').each(function(e) {
          if(this.checked == true) {
            labelsChecked.push($(this).siblings('label').text());
          }
        });

        if(!$('.selected-market-updates').length) {
          var displayedSelections = '<div class="selected-market-updates"><div class="selected-market-updates__inside"><div class="selected-market-updates__text">';
          if(labelsChecked.length === 0) {
            displayedSelections += Drupal.t('Showing All Market Update Categories');
          } else {
            displayedSelections += Drupal.t('Showing ') + '<span>';
            for(var i = 0; i < labelsChecked.length; i++) {
              if(i === 0) {
                displayedSelections += labelsChecked[i];
              } else if(i === labelsChecked.length - 1) {
                displayedSelections += ' ' + Drupal.t('and') + ' ' + labelsChecked[i];
              } else {
                displayedSelections += ', ' + labelsChecked[i];
              }
            }
            displayedSelections += '</span>';
          }
          displayedSelections += '</div></div></div>';
          $(displayedSelections).prependTo($('.wrapper--selected-market-updates'));
        }
      });
      }, 10);
    }
